...
def login():
    if request.method == "POST":
        ...
        if len(name) > 100:
            flash('Der Benutzername darf maximal 100 Zeichen lang sein.', 'error')
            return render_template("login.html")
...
def create_inserat():
    ...
    if request.method == "POST":
        ...
        if len(subject) > 100:
            flash('Das Fach darf maximal 100 Zeichen lang sein.', 'error')
            return render_template("create_inserat.html")
        if len(contact_info) > 100:
            flash('Die Kontaktinformationen dürfen maximal 100 Zeichen lang sein.', 'error')
            return render_template("create_inserat.html")
        try:
...
def edit_user(user_id):
    ...
    if request.method == "POST":
        ...
        if len(new_name) > 100:
            flash('Der Name darf maximal 100 Zeichen lang sein.', 'error')
            return render_template("edit_user.html", user=user)
